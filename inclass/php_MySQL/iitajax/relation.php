...
<?php
  if ($dbOk) {
    ...
    $query = 'select `movies`.`title`, `actors`.`last_name`, `actors`.`first_names`, `actors`.`dob` from `movies`, `actors`, `movie actors` where `movie actors`.`movieid` = `movies`.`movieid` and `movie actors`.`actorid` = `actors`.`actorid` order by `movies`.`movieid`';
    $result = $db->query($query);
    $numRecords = $result->num_rows;
    
    echo '<tr><th>Title:</th><th>Name:</th><th>Data:</th></tr>';
